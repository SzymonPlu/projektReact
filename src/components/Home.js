...
function Home() {
  ...
  const handleSearch = (event) => {
    ...
    const filteredItems = mediaItems.filter(item => {
      ...
      return (keywords.includes(query) || name.includes(query)) && duration >= durationFilter.min && duration <= durationFilter.max;
    });
  ...
  const handleDeleteSelected = async () => {
    if (selectedItems.length === 0) {
      alert('Nie wybrano żadnych elementów do usunięcia');
      return;
    }
  
    const confirmation = window.confirm(`Czy na pewno chcesz usunąć ${selectedItems.length} element(y)?`);
    if (!confirmation) {
      return;
    }
  
    console.log('Usuwanie elementów o ID:', selectedItems);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Błąd podczas usuwania zaznaczonych elementów');
      }
      ...
      console.error('Błąd podczas usuwania zaznaczonych elementów:', error);
    }
  };
  ...
  return (
    <div>
      ...
      {/* Wyświetlamy formularz ustawień jeśli showSettingsForm jest ustawione na true */}
      {showSettingsForm && (
        <div className="settings-form" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: 'white', padding: '20px', borderRadius: '5px', boxShadow: '0px 0px 10px rgba(0, 0, 0, 0.1)', zIndex: '9999' }}>
          {/* Tutaj umieść formularz ustawień */}
          <h2>Settings</h2>
          {/* Na przykład: */}
      ...
      <div className="content-wrapper" style={{ display: 'flex' }}>
        <div className="left-panel" style={{ width: '30%', backgroundColor: '#D3D3D3', padding: '15px', marginRight: '15px' }}>
